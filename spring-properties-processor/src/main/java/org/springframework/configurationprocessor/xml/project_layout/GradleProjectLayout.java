...
public class GradleProjectLayout implements ProjectLayout {
    @Override
    ...
    public List<Path> xmlLocations(ProcessingEnvironment processor) {
        ...
        Path path;
        path = Path.of(moduleDir, "build/resources/main");
    ...
    private Path getModuleDir(ProcessingEnvironment processor) {
        try {
            ...
            // D:\proto\spring-configuration-processor\test-project\build\classes\java\main\tmp
            ...
            int i = classOutputPathString.indexOf(File.separator + "build" + File.separator); //Gradle
            if (i >=0) {
